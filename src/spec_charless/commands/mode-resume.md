> Referencia de **charless-ia** — Modo Reanudación. Se carga cuando ya existe `.skill-state.json` (o `CLAUDE.md`/`TODO.md`/`SPEC.md`) en el proyecto y el usuario quiere continuar donde quedó.
...
4. **Si el usuario dice algo tipo "el MVP está listo", "cerremos esta fase", "arranquemos la v2"** → aplicar el mecanismo de snapshot de `.charless/reference/methodologies.md` sección "Snapshots de fase — carpeta `specs/`": congelar el `SPEC.md` actual en `specs/<fase>/SPEC.md` antes de seguir editando el spec vivo para lo que sigue.
