...
Este archivo documenta las metodologías de desarrollo que usa la skill `/charless-ia` al crear proyectos. Cada proyecto generado aplica **SDD** para definir el alcance, **DDD** para modelar el dominio (proyectos con backend/DB), **TDD** durante el desarrollo de cada feature, y **BDD** para documentar comportamientos en iteraciones futuras.
...
**`charless-ia` implementa SDD en el nivel Spec-Anchored.** `SPEC.md` se genera en P1.7 (o MA-3 en Modo Adopción) y se actualiza — no se re-escribe desde cero — cada vez que el alcance cambia. Ver `.charless/templates/SPEC.md.template` (incluye una sección de Historial de cambios) y `AGENTS.md` del proyecto generado, sección "Agregar o modificar features (Spec-Anchored)", para el mecanismo exacto.
...
Al crear un proyecto con `/charless-ia`, se genera un `SPEC.md` en la raíz que define:
