> Referencia de **rocky-spec** — Modo Adopción. Se carga cuando el directorio tiene código pre-existente pero SIN `.skill-state.json` (proyecto no creado con esta skill).
...
| **Branching** | `git branch -a` → ¿existe `development`? `git log --oneline -20` → ¿los commits recientes son directo sobre `main`/`development`, o vienen de ramas `feature/*`/`fix/*` mergeadas? |
| **Archivos de skill** | `AGENTS.md`, `CLAUDE.md`, `SPEC.md`, `TODO.md`, `design-system/MASTER.md` — qué ya existe |

Si no existe `development`, se crea en MA-6 al generar `AGENTS.md` (queda como tarea en el TODO si el usuario prefiere hacerlo después). Si el proyecto adoptado ya tiene una rama `dev` con historia propia, no se renombra de oficio — se reporta como hallazgo y se le pregunta al usuario si prefiere migrar a `development` o mantener `dev`. Si los commits recientes son mayormente directo sobre ramas principales, es un hallazgo 🟡 — se reporta en MA-2 igual que salud del código/seguridad/observabilidad, no se fuerza un cambio de hábito sin avisar primero.

### MA-1.5 · Health check — tamaño de archivos y code smells

**Si `rocky` está disponible** (`pip install rocky-spec` o `uvx rocky-spec`), correr `rocky check code .` — es la versión determinista y probada de este mismo chequeo (ver `health_check.check_file_sizes`), evita repetir el heurístico de abajo a mano. **Si no está disponible**, aplicar el criterio manual:
...
Clasificar cada archivo del top 15 contra la tabla de `.rocky-spec/reference/coding-principles.md` sección "Tamaño de archivo — límites y cuándo dividir":
...
# Extensión — gestión de dependencias (.rocky-spec/reference/dependencies.md), no solo seguridad:
...
- 🟡 **Revisar** — `.gitignore` sin `.env`, vulnerabilidades `high`/`critical` en `npm audit`, dependencias sin usar acumuladas, o una licencia GPL/AGPL colada en un proyecto que se redistribuye (ver `.rocky-spec/reference/dependencies.md` "Compliance de licencias de terceros" — no aplica si el proyecto es privado).
...
> Div clickeable sin rol, botón solo-ícono sin `aria-label`, y contraste WCAG requieren parsear el tag completo o calcular luminancia — no hay un heurístico manual confiable para esos tres sin `rocky` instalado. Si no está disponible, avisar al usuario que esos tres quedan sin auditar en este chequeo.

Clasificar hallazgos:
- 🟡 **Revisar** — cualquier resultado de los heurísticos de arriba (o de `rocky check accessibility` si está disponible).
...
Usar la misma tabla de decisión de P1.7 (fullstack/backend → sí; frontend puro → no) — ver `.rocky-spec/commands/p1-spec-ddd.md`, Paso A.
...
- Si no hay nada que inferir → hacer las 3 preguntas del Paso B de P1.7 igual que en el flujo normal (ver `.rocky-spec/commands/p1-spec-ddd.md`, Paso B)

**Paso C — Generar SPEC.md:**
Generar el SPEC.md igual que en P1.7 (ver `.rocky-spec/commands/p1-spec-ddd.md`, Paso C), pero:
...
Si el proyecto tiene frontend, ejecutar P4.5 normalmente (ver `.rocky-spec/commands/p4.5-design-system.md`), usando el mismo `.rocky-spec/templates/MASTER.md.template`. Pero antes:
...
|---|---|---|
| `CONSTITUTION.md` | `.rocky-spec/templates/CONSTITUTION.md.template` | Arquitectura y patrones detectados en MA-1, principios del perfil, reglas base de `coding-principles.md`/`security.md` |
| `AGENTS.md` | `.rocky-spec/templates/AGENTS.md.template` | Stack detectado, arquitectura documentada, principios del perfil, comandos reales del proyecto — incluyendo el Spec Drift Check (mismo criterio que P6: buscar el backend/ORM detectado en MA-1 dentro de `.rocky-spec/reference/stacks-code.md`; si no está en la tabla, derivarlo del conocimiento del framework y ofrecer agregarlo a la tabla; solo deshabilitar si no hay confianza real en la sintaxis) |
| `CLAUDE.md` | `.rocky-spec/templates/CLAUDE.md.template` | Importa `AGENTS.md` + roles del asistente |
| `SECURITY.md` | `.rocky-spec/templates/SECURITY.md.template` | Stack y auth detectados en MA-1, hallazgos de MA-1.6 (los ítems OWASP con hallazgo 🔴/🟡 arrancan sin marcar, no asumidos como resueltos) |
| `OBSERVABILITY.md` | `.rocky-spec/templates/OBSERVABILITY.md.template` | Stack detectado en MA-1, hallazgos de MA-1.7 (si ya hay error tracking/health check, documentarlos tal cual están en vez de asumir que no existen) |
| `ACCESSIBILITY.md` | `.rocky-spec/templates/ACCESSIBILITY.md.template` | **Solo si el proyecto detectado en MA-1 tiene interfaz visual.** Hallazgos de MA-1.8 (los ítems del checklist con hallazgo 🟡 arrancan sin marcar). Condicional como `design-system/MASTER.md` — no pisar si el proyecto no tiene UI |
| `PRACTICES.md` | `.rocky-spec/templates/PRACTICES.md.template` | **Opcional — no es parte del audit de MA-1 ni de `rocky check drift`**: ofrecerlo ("¿Querés dejar escrito qué principios, patrones y herramientas aplica este proyecto?") en vez de generarlo sin preguntar. Se arma como P5.9 (`.rocky-spec/commands/p5.9-practices.md`, generado con `rocky build --template`), pero los patrones activos son **los que el código ya usa** (detectados en MA-1; cada ficha de `.rocky-spec/reference/architecture-styles/` trae sus señales), no los que convendría usar — no documentar como práctica algo que el proyecto no sigue |
| `CHANGELOG.md` | `.rocky-spec/templates/CHANGELOG.md.template` | Si hay tags de git existentes, usarlos para reconstruir versiones pasadas (ver caso especial abajo); si no hay historial de versiones, arranca en `[Unreleased]` |
| `LICENSE` | `.rocky-spec/templates/LICENSE-*.template` | Solo si no existe ya (ver caso especial abajo) — nunca inventar una licencia sin preguntar |
| `TODO.md` | `.rocky-spec/templates/TODO.md.template` | Features del SPEC.md (las ✅ ya marcadas como hecho, las pendientes como `- [ ]`) |
| `design-system/MASTER.md` | `.rocky-spec/templates/MASTER.md.template` | Resultado de MA-5 (tokens detectados o generados desde cero) |
...
1. **Backup + regenerar, en un solo comando**: `rocky build . --values .rocky-spec/build-values.json --template <Archivo>.md.template --output <Archivo>.md --update` (RF-23) — hace el backup (`_<Archivo>.md`) y renderiza el archivo fresco, y reporta qué encabezados quedaron **solo en el backup** (candidatos a portar) y cuáles trajo el template **nuevos**. Usa el `build-values.json` que persiste `rocky build` (ver P6 en `commands/p6-p7-files-todo.md`); si el proyecto es de antes de que existiera esa persistencia y `--update` falla porque no hay valores suficientes, reconstruir el JSON a mano con los valores que ya estén en el backup antes de reintentar, no adivinar.
2. **Mergear**: para cada encabezado que el comando reportó como "solo en el backup", decidir si portarlo al archivo nuevo (decisiones del proyecto, gotchas propios, casos de referencia editados a mano) — y para cada encabezado que **ya existía en ambos** (mismo título, no aparece en ningún reporte de "solo en"), **copiar el contenido del archivo recién generado tal cual, no parafrasear ni reconstruir de memoria**. Mostrarle el resultado al usuario antes de borrar el backup — igual que la opción "mergear" del resto de esta tabla.

   > **Por qué esta distinción importa** (caso real, RF-23): un `CONSTITUTION.md` adoptado conservó el título "Artículo 7 — Versionado y releases" pero perdió la tabla normativa acción→mecanismo (`AskUserQuestion` para push/merge/tag/borrado de rama) porque el merge la resumió en prosa en vez de copiarla literal del archivo fresco. El comando `--update` no detecta este caso — compara títulos, no contenido — así que la responsabilidad de copiar literal el contenido de secciones que ya coincidían en título es 100% del agente en este paso.
3. **Borrar el backup** solo con confirmación explícita del usuario, una vez que confirma que el merge quedó bien.
...
- [ ] 🔴 Dividir `src/pages/Dashboard.tsx` (842 líneas) — ver `.rocky-spec/reference/coding-principles.md` sección "Tamaño de archivo"
