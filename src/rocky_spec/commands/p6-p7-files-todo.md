> Referencia de **rocky-spec** — Pasos P6 y P7 del flujo de creación: archivos base del proyecto y TODO colaborativo.

### P6 · Genera archivos base

Crear, dentro del directorio del proyecto, copiando desde `.rocky-spec/templates/`:

**Si `rocky` está disponible**: en vez de copiar cada `.template` y reemplazar `{{PLACEHOLDER}}` a mano, volcar todos los valores recolectados hasta acá (stack de P3, tipo/descripción de P1, decisiones de P5.6/P5.7, licencia elegida más abajo, etc.) a un JSON plano y correr `rocky build`:

```bash
# .rocky-spec/build-values.json
{
...
```
```bash
rocky build . --values .rocky-spec/build-values.json
```
...
- `OBSERVABILITY.md` (desde `OBSERVABILITY.md.template`, con las decisiones de P5.7 — ver `.rocky-spec/commands/p5.7-observability.md`)
- `CHANGELOG.md` (desde `CHANGELOG.md.template`, formato Keep a Changelog + SemVer — ver `.rocky-spec/reference/versioning.md`)
...
**Reglas base — siempre activas, no dependen del perfil** (ver `.rocky-spec/reference/coding-principles.md` sección "Reglas base — no son opt-in"):
...
**Principios de código del perfil** (opt-in, ver `.rocky-spec/reference/coding-principles.md`): cuando la skill genera código de scaffolding (componentes ejemplo, configs, archivos base), respetar los principios activos del perfil:
...
3. Para el resto, buscar el backend y el ORM/DB de este proyecto en las tablas de `.rocky-spec/reference/stacks-code.md` sección "Patrones de detección — Spec Drift Check".
...
5. **Si NO están en la tabla** (framework nuevo, poco común, o directamente no está cargado todavía) → no deshabilitar de entrada. Derivar el patrón ahí mismo, igual que se razonaría para explicarle a alguien cómo se ve una ruta o un modelo en ese framework (ej. si es Elysia: `.get(`/`.post(` sobre una instancia de `new Elysia()`; si es tRPC: `.query(`/`.mutation(` dentro de un router). Usar ese patrón derivado para completar el placeholder.
   ...
   - Si confirma → agregar la fila a la tabla correspondiente de `.rocky-spec/reference/stacks-code.md` (mismo formato que las existentes). La tabla crece con el uso real, igual que `project-patterns/` con los patterns de proyecto completo.
...
2) Por features (cada tarea = una historia de punta a punta)
   ...
   .rocky-spec/reference/methodologies.md).
```
...
**Por capas** → las tareas de "Features iniciales" (o los archivos de `todos/` en modo orquestador) se agrupan en `Dominio / Base de datos` → `API / Backend` → `Frontend / UI`, en ese orden (ver razonamiento DDD en `.rocky-spec/reference/methodologies.md`).
...
Un `TODO.md` con todas las secciones inline funciona bien al principio, pero en un proyecto fullstack activo puede terminar siendo el mismo problema de God File que ya resolvimos para código (ver `.rocky-spec/reference/coding-principles.md` sección "Tamaño de archivo") — solo que en la documentación. La skill usa el mismo criterio de tamaño de archivo Markdown (`.rocky-spec/reference/coding-principles.md`: ideal <500 líneas, dividir sí o sí a partir de 500) aplicado de forma **proactiva**, no solo reactiva.
...
2. Generar un archivo por grupo usando `.rocky-spec/templates/todo-group.md.template`:
