...
> El **escaneo de vulnerabilidades** (`npm audit`, Dependabot, etc.) ya vive en `.rocky-spec/reference/security.md` — es la mitad de seguridad de este tema. Este archivo cubre la otra mitad: mantener las dependencias sanas en el tiempo (no desactualizadas, no infladas, no con licencias incompatibles) — es mantenimiento, no seguridad.
...
- **P6 (Genera archivos base)**: rellena la sección "Gestión de dependencias" de `AGENTS.md` y la configuración de Dependabot/Renovate (extiende la de `.rocky-spec/reference/security.md`, no la duplica) con la cadencia y política elegidas.
...
|---|---|---|
| Librerías de aplicación (`dependencies`) | Rango caret (`^1.2.3`) | Recibe patches y minors automáticamente — la mayoría son seguros por SemVer (ver `.rocky-spec/reference/versioning.md`) |
...
**Configuración de Dependabot** (extiende la de `.rocky-spec/reference/security.md`, no la reemplaza):
...
Con el tiempo, un proyecto acumula dependencias que ya no se importan en ningún lado — pesan en el bundle, en el tiempo de instalación, y en la superficie de ataque (más código de terceros = más riesgo, ver `.rocky-spec/reference/security.md`).
