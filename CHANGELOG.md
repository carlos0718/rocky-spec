...
- **La nota que acompaña al ancla reparada es texto fijo, no prosa redactada al momento** — la primera reparación manual de este mismo `CLAUDE.md` (antes de que existiera `ensure_claude_md_anchor()`) llevó una nota explicativa distinta a la que terminó escribiendo el código. Ahora ambas usan la misma constante (`CLAUDE_MD_ANCHOR_NOTE`), más corta que la de `CLAUDE.md.template` a propósito — no promete la sección de roles de expertise porque la reparación no la agrega.
