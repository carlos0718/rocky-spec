...
- **Referencia `design-patterns.md`** — los 23 patrones de GoF más Repository, Dependency Injection y Registry/Plugin, cada uno con cuándo sí, cuándo no, señal en el código, ejemplo mínimo y marca de frecuencia (frecuente/ocasional/raro) para no sobreaplicarlos. Un índice con la tabla de los 23 y una guía "tengo este problema → considerar este patrón", y el detalle partido en `design-patterns/creational.md`, `structural.md` y `behavioral.md` para abrir solo el tipo que hace falta. (US-34, RF-26)
- **Referencia `best-practices-frontend.md`** — buenas prácticas de frontend agnósticas de framework (componentes, estado, rendimiento, datos y red, errores, seguridad del cliente, testing, tipado), cada una con su señal de falla y su "cuándo NO", y una tabla de herramientas típicas que alimentará el `PRACTICES.md` de P5.9. No repite estilos inline/HTML semántico (`coding-principles.md`) ni diseño visual (`ui-design-guidelines.md`). (US-34, RF-26)
- **Referencia `best-practices-backend.md`** — buenas prácticas de backend agnósticas de lenguaje y framework (capas y límites, validación en el borde, manejo de errores, diseño de API con idempotencia y paginación, datos y persistencia, configuración, trabajo asíncrono, testing), con señal de falla y "cuándo NO" en cada una y una tabla de herramientas típicas para el `PRACTICES.md` de P5.9. Seguridad y observabilidad apuntan a `security.md` y `observability.md` en vez de duplicarse. (US-34, RF-26)
- **Fichas de arquitectura `monolith.md` y `layered.md`** en `reference/architecture-styles/` — una ficha por estilo con analogía sin jerga, diagrama, estructura de carpetas de ejemplo, cuándo sí/no, señales de alarma, qué se gana y qué se sacrifica, cómo se combina con otros estilos y cómo la usa la skill en P4 y Modo Adopción. La monolítica distingue plana de modular (paso previo a microservicios); la de capas distingue capas de tiers y explica cómo evolucionar a Onion/Hexagonal invirtiendo la dependencia de datos. (US-34, RF-26)
- **Fichas de arquitectura `onion.md` y `hexagonal.md`** — Onion con sus cuatro anillos y la regla de dependencia hacia adentro; Hexagonal con la distinción entre puertos y adaptadores del lado que maneja la app y del lado manejado por la app. Las dos incluyen las señales de alarma propias (dominio que importa el ORM, puertos que copian la API del proveedor) y `onion.md` suma la tabla comparativa Onion/Clean/Hexagonal. (US-34, RF-26)
- **Fichas de arquitectura `microservices.md` y `event-driven.md`** — microservicios con sus prerrequisitos (CI/CD por servicio, observabilidad madura, dominio entendido), las señales del monolito distribuido y los conceptos a resolver (datos propios, sagas, contratos versionados); orientada a eventos con la diferencia entre evento y comando, consistencia eventual, idempotencia, outbox y dead-letter queue, tratada como complemento de otro estilo y no como estilo base. P4 no recomienda microservicios por defecto: propone monolito modular salvo escala, equipo e infraestructura que lo justifiquen. (US-34, RF-26)
- **`architectures.md` pasa a ser el mapa de los estilos** — cada uno de los seis con ficha apunta a la suya (`monolith`, `layered`, `onion`, `hexagonal`, `microservices`, `event-driven`), suma la sección "La regla de dependencia" que comparten Clean, Hexagonal y Onion, la fila "monolito modular" en la matriz de decisión y el "cuándo NO" de Onion, que no lo tenía. `p4-architecture.md` y `mode-adopt.md` abren solo la ficha del estilo elegido o detectado; P4 recomienda microservicios únicamente con escalabilidad alta, equipo de 4+ e infraestructura, y trata event-driven como complemento. (US-34, RF-26)
...
- **`rocky check drift` no reconocía como "con interfaz visual" a la mayoría de los proyectos con UI** — solo miraba `.html`, `.jsx` y `.tsx`: un proyecto adoptado con vistas Razor (`.cshtml`, `.razor`), Rails (`.erb`, `.haml`), Laravel (`.blade.php`), Vue/Svelte/Astro, Node (`.ejs`, `.hbs`, `.njk`, `.pug`), Python (`.jinja`, `.j2`), `.twig`, `.jsp`, `.liquid`, `.mustache` o `.gohtml` nunca recibía el aviso de que faltaban `ACCESSIBILITY.md` y `design-system/MASTER.md`. Ahora los reconoce. `.blade.php` se distingue por el nombre completo, así una API PHP sin vistas sigue sin contar como UI. La comparación de extensiones ahora no distingue mayúsculas (`INDEX.HTML`, `Big.CS`).
...
- **`rocky check code` no escaneaba varios stacks que el kit soporta** — sumó `.vue`, `.svelte`, `.java`, `.kt`, `.cs`, `.rb` y `.php` (un archivo de 900 líneas en cualquiera de ellos daba "sin hallazgos"), y dejó de escanear carpetas generadas o de terceros (`venv`, `vendor`, `obj`, `target`, `.next`, `coverage`).
...
- **Remediación guiada para el drift de contenido — nuevo caso especial "drift de contenido" en MA-6 (`mode-adopt.md`)**: backup del archivo actual, regenerarlo con `rocky build --force` usando `build-values.json`, y mergear sección por sección antes de borrar el backup. `mode-resume.md` distingue este caso (por el texto del hallazgo) del de archivo ausente y lo resuelve siguiéndolo en vez de intentar generar de cero un archivo que ya existe. El mensaje de próximo paso de `rocky check drift`/`rocky update` ya no asume "generar lo que falte" para todos los hallazgos. (US-28, RF-22)
...
### Fixed
- **`charless check qa` no detectaba placeholders con sintaxis `NOMBRE, default: valor`** que sobrevivían sin rellenar en el archivo final — `qa_review.py` tenía su propio regex, separado y más simple que el de `render_template.py`, que solo reconocía `NOMBRE` a secas. Unificado: ahora reusa `render_template.find_unresolved()`, la misma fuente de verdad para "qué es un placeholder sin resolver" en todo el proyecto.
