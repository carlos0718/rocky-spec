...
| RF-10 | P1 (should) | Confirmaciones explícitas del humano en operaciones de git | Tabla normativa acción→mecanismo (Artículo 7 de `CONSTITUTION.md`) para push/merge/tag/borrado de ramas, con enforcement real instalado en `.claude/settings.json` del proyecto destino (no solo la regla escrita en prosa) |
| RF-11 | P2 (nice) | Documentación de comandos del agente | `rocky commands` muestra los dos niveles — CLI (`rocky init`, `rocky check`...) y agente (`/rocky-*`) — con conteo derivado de los archivos reales, no hardcodeado |
| RF-12 | P2 (nice) | Seguimiento de servicios externos | P3 captura servicios de terceros (email, pagos, storage, auth...) al confirmar el stack; `AGENTS.md`/`TODO.md` generan su sección y tareas de seguimiento por servicio |
| RF-13 | P1 (should) | Reparación no destructiva de anclas al conocimiento compartido | `CLAUDE.md`/`rocky.mdc` se auto-reparan si pierden el ancla (`@AGENTS.md` / puntero a `.rocky-spec/`) sin pisar el resto del archivo; `rocky check anchors` lo verifica en cualquier proyecto sin depender de correr `init` |
...
| RF-20 | P2 (nice) | Detección de drift en proyectos adoptados con versiones viejas | `rocky update` verifica, cuando `.skill-state.json` marca `"mode": "adopted"`, que los archivos que el Modo Adopción vigente genera en MA-6 (`CONSTITUTION.md`, `CHANGELOG.md`, `SECURITY.md`, `OBSERVABILITY.md` siempre; `ACCESSIBILITY.md` y `design-system/MASTER.md` si el proyecto tiene UI; `LICENSE` según el nuevo campo `license_decision`) existan en el proyecto — reporta como hallazgo cada uno ausente, igual que `rocky check anchors` con el ancla de `CLAUDE.md`, sin generarlo por su cuenta. MA-6 pasa a guardar `"license_decision"` (`"mit"`/`"apache2"`/`"proprietary"`/`"skipped"`) en `.skill-state.json` al responder la pregunta de `LICENSE` — así `rocky update` distingue "el usuario ya eligió no tener LICENSE" (`"skipped"`, no es drift) de "nunca se le preguntó porque la sesión es anterior a este campo" (ausencia del campo, si flaggea) |
...
- **US-13** (implementa RF-10): Como desarrollador que corre `rocky init --agent claude`, quiero que las reglas de confirmación del Artículo 7 se instalen en `permissions.ask` de `.claude/settings.json`, para que el enforcement exista de verdad y no solo la convención escrita
- **US-14** (implementa RF-11): Como desarrollador, quiero que `rocky commands` muestre también los comandos del agente (`/rocky-*`) además de los de la CLI, para no tener que buscarlos fuera de la terminal
- **US-15** (implementa RF-12): Como desarrollador, quiero que P3 me pregunte por servicios de terceros al confirmar el stack, para que "los mails los mando con Resend" genere tareas de seguimiento reales en vez de perderse en la conversación
- **US-16** (implementa RF-13): Como desarrollador, quiero que `CLAUDE.md`/`rocky.mdc` se reparen solos si pierden el ancla al conocimiento compartido, para no tener que regenerarlos enteros a mano y perder notas o roles personalizados
...
- **US-23** (implementa RF-20): Como desarrollador que adoptó un proyecto con una versión vieja de la skill, quiero que `rocky update` me avise si mi `.skill-state.json` quedó de una adopción anterior a que existiera algún archivo que MA-6 genera hoy (ej. `CHANGELOG.md`, `LICENSE`), para enterarme del hueco sin tener que compararlo a mano contra el flujo actual — sin que se confunda con una decisión mía real de no tener `LICENSE`
...
| 2026-09-09 | Nueva RF-20/US-23: `rocky update` detecta drift entre `.skill-state.json` de proyectos adoptados y los archivos que MA-6 genera en la versión vigente de la skill — pedido explícito del usuario tras encontrar un proyecto (`cuotify`) adoptado el 2026-08-10, antes de que existieran los pasos de `CONSTITUTION.md`/`SECURITY.md`/`OBSERVABILITY.md`/`CHANGELOG.md`/`ACCESSIBILITY.md` en Modo Adopción — auditoría manual del propio usuario confirmó las 5 ausencias comparando el commit de adopción contra `mode-adopt.md` vigente. Incluye `LICENSE`, agregando el campo `license_decision` a `.skill-state.json` para no confundir "el usuario ya eligió no tenerla" con "nunca se le preguntó" — pedido explícito del usuario, que prefirió incluirla en vez de excluirla | (pendiente) |
