...
**Para cambios breaking o de alcance grande, la misma pausa aplica antes del `push`, no solo antes del merge.** Señales de que un cambio entra en esta categoría: el commit lleva `BREAKING CHANGE:` en el footer, o toca la mayoría de los archivos del proyecto (un rename, una migración de framework, un refactor transversal). En esos casos: commitear localmente, parar, mostrar el mismo resumen que se usaría antes del merge, y esperar confirmación explícita antes de `git push`. Para el resto de los commits (el caso normal), el push sigue sin pausa — solo el merge se detiene. Si en un mismo momento hay más de una rama lista para pushear, no preguntar todo en un mensaje de texto: usar `AskUserQuestion` con una pregunta por rama (nombre de la rama + resumen corto en la descripción) y opciones Sí/No, para capturar la decisión de cada rama por separado.
