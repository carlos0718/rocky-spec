...
**Por qué existe:** este documento acumula reglas de la forma "cuando pase X, hacer Y" (Spec Drift Check, TODO Drift Check, Branch Discipline Check, US-20, el paso 6 de limpieza de ramas, etc.). Dos reglas con triggers parecidos pero no idénticos son fáciles de confundir en el momento de ejecutar — no porque el documento esté mal escrito, sino porque el agente ejecuta por reconocimiento de patrón ("recién pasó X, toca ofrecer algo") en vez de releer la regla exacta. Pasó en este repo: US-20 ("después de mergear `feature/*`/`fix/*` → `development`, ofrecer limpiar la sesión") y el paso 6 del release ("después de mergear a `master`, ofrecer borrar la rama") comparten el gesto "algo para ofrecer después de un merge a development", y se aplicó el contenido de la segunda en el momento de la primera.
...
> **No confundir con el paso 6 de "Cómo hacer un release" (limpieza de ramas), más abajo.** Ambas reglas se disparan cerca de un merge a `development`, pero son cosas distintas: esta es sobre el *historial de la conversación* y se ofrece apenas se confirma el merge `feature/*`/`fix/*` → `development`. La limpieza de ramas es sobre *borrar la rama de git* y recién se ofrece al final del flujo de release completo, después de que `master` ya quedó sincronizado — nunca apenas se mergea a `development`. Pasó una vez en este repo que se ofreció borrar la rama en el momento de US-20, salteándose el version check, el tag, el release y el merge a `master`.
...
- **Cómo hacer un release** cuando corresponda:
  ...
  6. Después de mergear a la rama principal: listar `git branch --merged development` (menos `development`/`master`) y preguntarle al usuario cuáles borrar (local + remoto) **con `AskUserQuestion`** — nunca borrar sin confirmar, y nunca ofrecer una rama que no esté 100% mergeada. Usar `git branch -d` y no `-D`: si la rama no estuviera mergeada, `-d` se niega y actúa como red de seguridad además de la verificación previa. **No confundir con US-20** (arriba) — esa es la limpieza de la *sesión*, dispara apenas se mergea `feature/*`/`fix/*` → `development`; esta es la limpieza de la *rama de git*, y solo corre acá, al final del release, nunca antes.
