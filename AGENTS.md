...
- **Cómo hacer un release** cuando corresponda:
  1. Mover `[Unreleased]` a `## [X.Y.Z] - {{fecha}}` en `CHANGELOG.md`, dejando un `[Unreleased]` vacío arriba. **En el mismo paso**, actualizar `version = "X.Y.Z"` en `pyproject.toml` (línea `version = "..."` bajo `[project]`) — el CHANGELOG documenta el release, pero `pyproject.toml` es lo que `rocky --version` y el banner de bienvenida leen en runtime; si quedan desincronizados, `uv tool upgrade` instala un número de versión que no coincide con el tag/Release de GitHub (pasó en este repo: tags y Releases llegaron a v0.17.0 con `pyproject.toml` todavía en v0.15.0, porque los releases v0.16.0 y v0.17.0 solo tocaron el CHANGELOG).
