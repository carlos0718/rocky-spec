...
### Trigger Ambiguity Check — redactar reglas nuevas sin triggers ambiguos

**Por qué existe:** este documento acumula reglas de la forma "cuando pase X, hacer Y" (Spec Drift Check, TODO Drift Check, Branch Discipline Check, el paso 6 de limpieza de ramas/sesión, etc.). Dos reglas con triggers parecidos pero no idénticos son fáciles de confundir en el momento de ejecutar — no porque el documento esté mal escrito, sino porque el agente ejecuta por reconocimiento de patrón ("recién pasó X, toca ofrecer algo") en vez de releer la regla exacta. Pasó en este repo: la recomendación de limpiar sesión ("después de mergear `feature/*`/`fix/*` → `development`, ofrecer limpiar la sesión") y el paso 6 del release ("después de mergear a `master`, ofrecer borrar la rama") compartían el gesto "algo para ofrecer después de un merge a development", y se aplicó el contenido de la segunda en el momento de la primera. La corrección inicial fue separarlas con más precisión en el texto; la corrección final (2026-09-10, ver "Cómo hacer un release" más abajo) fue notar que la separación no tenía un motivo de fondo — ambas son "cosas para ofrecer una vez que la integración terminó" — y unificarlas a propósito en la misma pausa, en vez de mantener dos triggers cercanos que solo existían separados por una distinción de trazabilidad (qué user story los originó), no por una diferencia real de comportamiento.

**Regla, para cuando se agrega una regla nueva con disparador basado en eventos** (un merge, un commit, un tag, un archivo tocado):

1. Antes de redactarla, buscar en el documento (`grep` por el sustantivo del evento — "merge", "commit", "tag", etc.) si ya existe otra regla con un disparador igual o parecido.
2. Si existe, dejar la diferencia explícita **en el texto de ambas reglas**, no solo en la cabeza de quien las escribió: qué las distingue (evento exacto, rama, orden relativo) y una referencia cruzada a la otra regla por nombre. No alcanza con que la diferencia "se entienda" leyendo las dos secciones por separado — tienen que decirlo una a la otra.
3. Esto es una auto-validación que corre el agente al redactar la regla nueva, no una aspiración de buena redacción — mismo estatus que el Spec Drift Check o el TODO Drift Check: se ejecuta como paso, no se asume.

**Límite de esto:** es un heurístico de redacción, no elimina la posibilidad de ejecutar mal una regla bien escrita — pero si el trigger está desambiguado en el texto, el costo de corregirlo en el momento (releer la línea exacta) es mucho menor que reconstruir la secuencia completa después del hecho.
...
- **Cómo hacer un release** cuando corresponda:
  1. Mover `[Unreleased]` a `## [X.Y.Z] - AAAA-MM-DD` en `CHANGELOG.md`, dejando un `[Unreleased]` vacío arriba. **En el mismo paso**, actualizar `version = "X.Y.Z"` en `pyproject.toml` (línea `version = "..."` bajo `[project]`) — el CHANGELOG documenta el release, pero `pyproject.toml` es lo que `rocky --version` y el banner de bienvenida leen en runtime; si quedan desincronizados, `uv tool upgrade` instala un número de versión que no coincide con el tag/Release de GitHub (pasó en este repo: tags y Releases llegaron a v0.17.0 con `pyproject.toml` todavía en v0.15.0, porque los releases v0.16.0 y v0.17.0 solo tocaron el CHANGELOG).
  ...
  5. **Release Sync Check — verificar que `master` quedó al día:** `git show origin/master:pyproject.toml | grep version` y comparar contra la `X.Y.Z` recién tageada. Si no coinciden (el tag/Release ya existen pero `master` todavía tiene una versión vieja de `pyproject.toml`), es la misma clase de gap que causó que `uv tool install`/`upgrade` instalara `0.15.0` con tags ya en `v0.17.0` — porque esos comandos clonan el HEAD del branch por defecto del remoto (`master`), no el tag. Ofrecer el merge `development → master` ahí mismo, **con `AskUserQuestion`** (mismo mecanismo que cualquier merge, ver "Branching" arriba) — no asumir que "ya se va a mergear después"; ese supuesto es justo lo que falló la vez que pasó esto. Este chequeo corre siempre que se taguea un release, no solo cuando hay sospecha de desfasaje.
  6. **Cierre del flujo — limpieza de rama y de sesión, en la misma pausa.** Después de mergear a la rama principal: listar `git branch --merged development` (menos `development`/`master`) y preguntarle al usuario cuáles borrar (local + remoto) **con `AskUserQuestion`** — nunca borrar sin confirmar, y nunca ofrecer una rama que no esté 100% mergeada. Usar `git branch -d` y no `-D`: si la rama no estuviera mergeada, `-d` se niega y actúa como red de seguridad además de la verificación previa.

     En esa misma pausa, ofrecer también limpiar la sesión actual (`AskUserQuestion` aparte) — el estado del trabajo vive en `TODO.md`/`SPEC.md`/`CHANGELOG.md`, no en la conversación, así que seguir arrastrando el historial de una integración ya cerrada es consumo de tokens sin beneficio:
     ...
     - Formato sugerido: *"El flujo ya terminó (merge a development, version check, release, merge a master). ¿Limpiamos la sesión antes de seguir?"* con opciones **"Sí, limpiar sesión"** (recomendada — la próxima tarea arranca leyendo `TODO.md`, no hace falta este historial) y **"No, seguir en esta sesión"** (por ejemplo si hay varias tareas chicas relacionadas para encadenar).
     - Si el nivel es 🔴 y el usuario elige "No": no repetir la pregunta en el mismo turno, pero sí avisar explícitamente que a ese nivel de consumo las respuestas pueden empezar a degradarse, y volver a ofrecer la limpieza apenas termine la siguiente tarea.
     - Es una recomendación, no un gate — nunca bloquear el flujo de git por esto.

     **Importante — este paso solo corre al final del flujo completo**, nunca apenas se mergea `feature/*`/`fix/*` → `development` con el version check/tag/release/merge a `master` todavía pendientes (ver "Trigger Ambiguity Check" más arriba — esta unificación fue una corrección deliberada, no la confusión que ese chequeo previene).
